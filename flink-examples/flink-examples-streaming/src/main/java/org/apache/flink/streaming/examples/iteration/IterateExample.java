...
public class IterateExample {
	...
	public static void main(String[] args) throws Exception {
		...
		DataStream<Tuple2<Tuple2<Integer, Integer>, Integer>> numbers = inputStream.map(new InputMap())
			.iterate(new LoopFunction<Tuple5<Integer, Integer, Integer, Integer, Integer>, Tuple2<Tuple2<Integer, Integer>, Integer>>() {
				@Override
				...
				loop(DataStream<Tuple5<Integer, Integer, Integer, Integer, Integer>> it) throws Exception {
					...
					return new Tuple2(step.select("iterate"), step.select("output").map(new OutputMap()));
				}
			});
