...
class CentralTracker(pathSummaries: java.util.Map[Integer, java.util.Map[Integer, PartialOrderMinimumSet]], maxScopeLevel: Integer, numberOfGlobalInstances: Integer, interval: Integer) extends Actor {
  ...
  def checkForCompleteness() : Unit = {
    if(!hasBeenSentOut && taskManagerCount.isDefined && localTrackers.size == taskManagerCount.get) {
      ...
      hasBeenSentOut = true
      for(actorRef <- localTrackers) {
        actorRef ! InitLocalTracker(localTrackers, pathSummaries, maxScopeLevel, numberOfGlobalInstances, interval)
      }
    }
  }
}
