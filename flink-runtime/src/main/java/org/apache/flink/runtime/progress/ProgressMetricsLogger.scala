...
class ProgressMetricsLogger(numWindows: Int, parallelism: Int, winSize: Long) extends Actor {
  
  private var LOG: Logger = LoggerFactory.getLogger(classOf[ProgressMetricsLogger])

  val printWriter = new PrintWriter(s"out/$numWindows-$parallelism-$winSize-${System.currentTimeMillis()}.csv");
